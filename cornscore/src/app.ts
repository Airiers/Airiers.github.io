...
if (url.searchParams.has("query")) {
    fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=${TMDB_API_KEY}&language=fr-FR&query=${url.searchParams.get("query")}&page=1`,
    )
        .then((res) => {
            return res.json();
        })
    ...
    fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=${TMDB_API_KEY}&language=fr-FR&query=${url.searchParams.get("query")}&page=2`,
    )
        .then((res) => {
            return res.json();
        })
    ...
    fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=${TMDB_API_KEY}&language=fr-FR&query=${url.searchParams.get("query")}&page=23`,
    )
        .then((res) => {
            return res.json();
        })
...
function putResults(response: Results) {
    const body = document.querySelector("body");
    ...
    if (resultList) {
        ...
        movies.forEach((movieDiv) => {
            movieDiv.addEventListener("mouseenter", () => {
                const movieId = (
                    movieDiv.querySelector("a") as HTMLAnchorElement
                )?.href.split("?movie=")[1];
    ...
    function setBackground(newBg: string) {
